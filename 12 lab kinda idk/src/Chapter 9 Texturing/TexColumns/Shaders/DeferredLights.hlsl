...
float DistributionBeckmann(float3 N, float3 H, float roughness)
{
    ...
    float numerator = exp((NdotH2 - 1.0) / (a2 * NdotH2));
    float denominator = PI * a2 * NdotH2 * NdotH2;
...
float G1_Beckmann(float NdotV, float roughness)
{
    float a = roughness;
    float a2 = a * a;
    ...
    float tanTheta = sqrt(1.0 - NdotV2) / NdotV;
    float a_tanTheta = a * tanTheta;
    if (a_tanTheta > 1.6)
        return 1.0;
    

    return (3.535 * a_tanTheta + 2.181 * a_tanTheta * a_tanTheta) /
           (1.0 + 2.276 * a_tanTheta + 2.577 * a_tanTheta * a_tanTheta);
}


float GeometrySmith_Beckmann(float3 N, float3 V, float3 L, float roughness)
{
    float NdotV = max(dot(N, V), 0.0);
    float NdotL = max(dot(N, L), 0.0);
    
    return G1_Beckmann(NdotV, roughness) * G1_Beckmann(NdotL, roughness);
}
...
float4 PS(VertexOut vo) : SV_Target
{
    ...
    if (LightType == 0) // direction
        ...
        if (DoCookTorrance == 1)
        {
            NDF = DistributionGGX(normal, halfVec, roughness);
            G = GeometrySmith(normal, toEyeW, lightDir, roughness);

        }
        else{
            NDF = DistributionBeckmann(normal, halfVec, roughness);
            G = GeometrySmith_Beckmann(normal, toEyeW, lightDir, roughness);
        }
        //float NDF = DistributionGGX(normal, halfVec, roughness);
        //float NDF = DistributionBeckmann(normal, halfVec, roughness);
        //float G = GeometrySmith_Beckmann(normal, toEyeW, lightDir, roughness);
