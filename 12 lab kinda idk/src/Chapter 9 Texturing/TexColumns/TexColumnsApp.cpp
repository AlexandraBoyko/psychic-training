...
XMFLOAT4 gGlobalDiffuse = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
XMFLOAT3 gGlobalFresnel = XMFLOAT3(0.0f, 0.0f, 0.5f);
...
void TexColumnsApp::Update(const GameTimer& gt)
{
	...
	ImGui::Text("Roughness/Metallic showcase");
	//ImGui::ColorEdit3("Diffuse Albedo", (float*)&gGlobalDiffuse);
	//ImGui::ColorEdit3("Fresnel R0", (float*)&gGlobalFresnel);

	if (ImGui::ColorEdit3("Diffuse Albedo", (float*)&gGlobalDiffuse) ||
		ImGui::ColorEdit3("Fresnel R0", (float*)&gGlobalFresnel)) {
		mMaterialsDirty = true;
	}
	ImGui::Text("PBR Formula");
	ImGui::Checkbox("CHECKBOX Cook-Torrance/Beckmann", &CookBeck);
...
void TexColumnsApp::Draw(const GameTimer& gt)
{
	...
	ID3D12DescriptorHeap* heaps[] = { m_ImGuiSrvDescriptorHeap.Get() };
...
void TexColumnsApp::UpdateMaterialCBs(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		if (mat->NumFramesDirty > 0)
		{
			...
			matConstants.DiffuseAlbedo = gGlobalDiffuse;   // один на всех
			matConstants.FresnelR0 = gGlobalFresnel;
...
void TexColumnsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.DoCookTorrance = CookBeck;
...
void TexColumnsApp::LoadTextures()
{
	...
	// Bricks
	LoadTexture("bricks_diffuse", L"../../Textures/bricks.dds");
	...
	LoadTexture("skyDiffuseCube", L"../../Textures/snowcube1024.dds", TextureType::CUBEMAP);
...
void TexColumnsApp::BuildRenderItems()
{
	...
	BuildRenderItem("box", "bricks0", XMMatrixScaling(8.5f, 0.1f,8.5f) * XMMatrixTranslation(-42.f, -5.f, -42.f), nullptr, 0, 1.f, 300.f);
