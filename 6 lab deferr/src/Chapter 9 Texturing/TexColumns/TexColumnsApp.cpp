...
float t = 0.05;
bool shootLights = true;
bool EpilepsyWarning = true;
...
void TexColumnsApp::Update(const GameTimer& gt)
{
	...
	t = 0.5f * (sin(gt.TotalTime() * 5.f));
	std::cout << t << std::endl;
	//XMMATRIX headRotation = XMMatrixTranslation(0.0f, t, 0.0f);
	XMMATRIX headRotation = XMMatrixRotationRollPitchYaw(0.0f, 3.14, t);
	...
	for (auto& rItem : mAllRitems)
	{
		...
		if (rItem->Name == "nigga2")
		{
			XMStoreFloat4x4(&rItem->World, XMMatrixScaling(3, 3, 3) * XMMatrixTranslation(5, 3+t, 30) * XMMatrixRotationRollPitchYaw(0,0,0));
			rItem->NumFramesDirty = gNumFrameResources;
		}
...
void TexColumnsApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	...
	for (auto& rItem : mAllRitems)
	{
		if (rItem->Name == "eyeL")
		{
			std::cout << rItem->Name << std::endl;
			XMFLOAT4X4 world = rItem->World;
			XMFLOAT3 positionL = {
				world._41,
				world._42,
				world._43
			};
			std::cout << positionL.x << "|" << positionL.y << "|" << positionL.z << std::endl;
		...
		if (rItem->Name == "eyeR")
		{
			...
			std::cout << positionR.x << "|" << positionR.y << "|" << positionR.z << std::endl;
...
void TexColumnsApp::UpdateLightCBs(const GameTimer& gt)
{
	...
	for (auto& l : mLights)
	{
		...
		if (l.type == 0)
		{
			...
			ImGui::Checkbox("Epilepsy Warning", &EpilepsyWarning);
			if (EpilepsyWarning) {
				l.Strength = t;
			}
			else {
...
void TexColumnsApp::BuildLights()
{
	...
	CreateSpotLight({ 4,3,30 }, { 0,0,-90 }, { 1,1,1 }, 1, 30, 6, 1);

	CreatePointLight({ -3,-100,0 }, { 4,0,0 }, 1, 5, 1);
	CreatePointLight({ 3,-100,0 }, { 4,0,0 }, 1, 5, 1);
}
...
void TexColumnsApp::BuildMaterials()
{
	...
	CreateMaterial("bricks2", 0, TexOffsets["textures/bricks2"], TexOffsets["textures/bricks2"], XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f), XMFLOAT3(0.05f, 0.05f, 0.05f), 0.3f);
}
...
void TexColumnsApp::BuildRenderItems()
{
	...
	boxRitem->Name = "box";
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(5.0f, 10.0f, 2.0f) * XMMatrixTranslation(7.0f, 5.0f, 20.0f));
	...
	boxRitem2->Name = "box";
	XMStoreFloat4x4(&boxRitem2->World, XMMatrixScaling(3.0f, 3.0f, 3.0f) * XMMatrixTranslation(-10.0f, 1.5f, 20.0f));
	XMStoreFloat4x4(&boxRitem2->TexTransform, XMMatrixScaling(1, 1, 1));
	boxRitem2->ObjCBIndex = 1;
	boxRitem2->Mat = mMaterials["bricks2"].get();
	...
	RenderCustomMesh("nigga2", "negr", "eye", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 0, 0), XMFLOAT3(0, 3, 25));
	RenderCustomMesh("nigga3", "negr", "NiggaMat", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 0, 3.14), XMFLOAT3(-5, 3, 35));

	RenderCustomMesh("eyeL", "left", "eye", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 3.14, 0), XMFLOAT3(0.6, 3.9, 1.1));
	RenderCustomMesh("eyeR", "right", "eye", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 3.14, 0), XMFLOAT3(-0.6, 3.9, 1.1));
	BuildFrameResources();
