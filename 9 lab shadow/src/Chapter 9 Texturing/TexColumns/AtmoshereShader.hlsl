// SkyAtmosphere.hlsl – атмосферное рассеяние для небесной сферы
...
VS_OUTPUT VS(VS_INPUT vin)
{
    ...
    // Мировая позиция: сфера находится в центре координат, без трансформации
    float3 worldPos = vin.PosL;
...
float4 PS(VS_OUTPUT pin) : SV_TARGET
{
    ...
    // Цветовой сдвиг плотности для Рэлея
    ...
    // Солнечный диск
    float sunAngularRadius = max(SunAngularRadius, 0.035f);
    ...
    // Закатные цвета
    if (sunElevation < 0.3f && sunElevation > -0.1f)
    {
